    ...
    <section className="py-20 bg-slate-50">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {tiers.map((t)=> (
            <div key={t.name} className={`rounded-2xl border p-6 shadow-sm bg-white ${t.highlight? 'ring-2 ring-brand-500':''}`}>
              <h3 className="text-lg font-semibold">{t.name}</h3>
              <p className="mt-4 text-3xl font-extrabold">{t.price}</p>
              <ul className="mt-4 space-y-2 text-sm text-slate-600">
                {t.points.map(p=> <li key={p}>✓ {p}</li>)}
              </ul>
              <a href="https://calendly.com/YOUR-SLUG/intro-call" className="mt-6 inline-flex w-full items-center justify-center rounded-xl bg-brand-600 px-4 py-2 font-semibold text-white hover:bg-brand-700">{t.cta}</a>
            </div>
          ))}
        </div>
      </div>
