...
async function ensureDb() {
  ...
  `);
  await query("CREATE INDEX IF NOT EXISTS competitor_scans_scanned_at_idx ON competitor_scans (scanned_at DESC)");
  dbReady = true;
}
...
async function listCompetitorsDb() {
  await ensureDb();
  const result = await query(
    "SELECT data FROM competitor_records ORDER BY updated_at DESC, created_at DESC"
  );
  return result.rows.map(rowToCompetitor).filter(Boolean);
}

async function writeCompetitorDb(competitor) {
  await ensureDb();
  await query(
    `INSERT INTO competitor_records (id, domain, name, data, created_at, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6)
    ...
    [
      ...
      competitor.name,
      competitor,
      competitor.createdAt || nowIso(),
      competitor.updatedAt || nowIso(),
    ]
  );
}

async function replaceCompetitorsDb(competitors) {
  await ensureDb();
  for (const competitor of competitors) {
    await writeCompetitorDb(competitor);
  }
}

async function listScansDb({ limit = 20 } = {}) {
  await ensureDb();
  const result = await query(
    "SELECT data FROM competitor_scans ORDER BY scanned_at DESC LIMIT $1",
    [limit]
  );
  return result.rows.map(rowToScan).filter(Boolean);
}

async function saveScansDb(scanResults) {
  await ensureDb();
  const newScans = scanResults.map((r) => ({ id: makeId("scan"), scannedAt: nowIso(), ...r }));
  for (const scan of newScans) {
    await query(
      `INSERT INTO competitor_scans (id, competitor_id, domain, scanned_at, data)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (id) DO UPDATE SET data = EXCLUDED.data, scanned_at = EXCLUDED.scanned_at`,
      [scan.id, scan.competitorId || null, scan.domain || null, scan.scannedAt, scan]
    );
  }
  await query(`
    DELETE FROM competitor_scans
    WHERE id NOT IN (
      SELECT id FROM competitor_scans ORDER BY scanned_at DESC LIMIT 100
    )
  `);
  return newScans;
}
...
export async function listCompetitorsAsync() {
  if (hasDatabaseConfig()) return listCompetitorsDb();
  if (!hasGitHubWriteConfig()) return listCompetitors();
...
export async function upsertCompetitorsAsync(items = []) {
  if (hasDatabaseConfig()) {
    const all = await listCompetitorsDb();
    ...
    await replaceCompetitorsDb(Array.from(byDomain.values()));
...
export async function updateCompetitorAsync(id, patch) {
  if (hasDatabaseConfig()) {
    ...
    const next = buildCompetitor({ ...existing, ...patch, id }, existing);
    await writeCompetitorDb(next);
    return next;
  }

  if (!hasGitHubWriteConfig()) return updateCompetitor(id, patch);
  const { value: all, sha } = await readRemoteJson(REMOTE_COMPETITORS_PATH, []);
  const idx = all.findIndex((c) => c.id === id);
  if (idx === -1) return null;
  all[idx] = buildCompetitor({ ...all[idx], ...patch, id }, all[idx]);
...
export async function removeCompetitorAsync(id) {
  if (hasDatabaseConfig()) {
    await ensureDb();
    await query("DELETE FROM competitor_records WHERE id = $1", [id]);
    return;
  }

  if (!hasGitHubWriteConfig()) return removeCompetitor(id);
  const { value: all, sha } = await readRemoteJson(REMOTE_COMPETITORS_PATH, []);
...
export async function listScansAsync({ limit = 20 } = {}) {
  if (hasDatabaseConfig()) return listScansDb({ limit });
  if (!hasGitHubWriteConfig()) return listScans({ limit });
...
export async function saveScansAsync(scanResults) {
  if (hasDatabaseConfig()) return saveScansDb(scanResults);
  if (!hasGitHubWriteConfig()) return saveScans(scanResults);
...
export async function getCompetitorStatsAsync() {
  if (hasDatabaseConfig()) {
    const [competitors, scans] = await Promise.all([
      listCompetitorsDb(),
      listScansDb({ limit: 100 }),
    ]);
    const lastScan = scans[0]?.scannedAt || null;
    return { total: competitors.length, scans: scans.length, lastScan };
  }

  if (!hasGitHubWriteConfig()) return getCompetitorStats();
  const [competitors, scans] = await Promise.all([
