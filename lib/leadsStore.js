...
export async function upsertLeadByLinkedIn(input) {
  const linkedinUrl = String(input.linkedinUrl || "").toLowerCase().trim();
  if (!linkedinUrl) {
    return createLead(input);
  }

  const leads = await listLeads();
  const existing = leads.find((lead) => String(lead.linkedinUrl || "").toLowerCase().trim() === linkedinUrl);
