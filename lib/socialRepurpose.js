...
export function getActiveSocialPlatforms() {
  return SOCIAL_PLATFORMS.filter((platform) => platform !== "reddit" || envFlag("SOCIAL_ENABLE_REDDIT"));
}
...
export async function repurposeBlogPost({ title, excerpt = "", content, slug = "" }) {
  ...
  const includeReddit = getActiveSocialPlatforms().includes("reddit");
  const redditUrl = includeReddit ? buildTrackedBlogUrl(slug, "reddit") : "";
  ...
  const [linkedin, x, facebook, bluesky, reddit] = await Promise.all([
    ...
    includeReddit ? repurposeForReddit(title, excerpt, content, redditUrl) : Promise.resolve(null),
  ]);
  ...
  if (includeReddit && reddit) {
    variants.reddit = normalizeVariant("reddit", { ...reddit, text: appendTrackedLink("reddit", reddit?.text, slug) });
  }
...
export async function moderateVariants({ variants, title = "", excerpt = "" }) {
  const moderatedEntries = await Promise.all(
    getActiveSocialPlatforms().filter((platform) => variants[platform]).map(async (platform) => [
      platform,
