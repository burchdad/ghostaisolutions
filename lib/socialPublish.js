...
async function getFacebookPublishCredentials() {
  ...
  if (!metaConnection?.accessToken) {
    ...
    return { pageId, pageAccessToken: fallback.token || "", source: fallback.source || "missing" };
  }
  ...
  return { pageId, pageAccessToken: "", source: "missing" };
}
...
export async function publishVariants({ platform, content, linkedinContent, xContent, facebookContent, blueskyContent, redditContent, redditTitle }) {
  ...
  if (platform === "facebook" || platform === "all") {
    ...
    results.facebook = await publishToFacebook(facebookContent || content, facebook.pageAccessToken, facebook.pageId);
  }
