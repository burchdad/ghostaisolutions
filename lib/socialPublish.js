...
import { getProviderConnectionAsync, getToken, saveTokensAsync } from "@/lib/tokenStore";
...
async function publishToFacebook(content, pageAccessToken, pageId, credentialError = "") {
  if (!pageAccessToken || !pageId) {
    return { success: false, error: credentialError || "Missing Facebook credentials" };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const details = [error.error?.message, error.error?.code ? `code ${error.error.code}` : "", error.error?.error_subcode ? `subcode ${error.error.error_subcode}` : ""]
        .filter(Boolean)
        .join(" | ");
      return { success: false, error: details || "Facebook API error" };
    }
...
async function validateFacebookPageToken(pageAccessToken, pageId) {
  if (!pageAccessToken || !pageId) return { ok: false, error: "Missing Facebook credentials" };

  const response = await fetch(
    `https://graph.facebook.com/v18.0/${pageId}?fields=id,name&access_token=${encodeURIComponent(pageAccessToken)}`,
    { cache: "no-store" }
  );
  ...
  return {
    ...
    error: data?.error?.message || `Facebook token check failed (${response.status})`,
  };
}

async function getFacebookPublishCredentials() {
  const orgId = "default";
  ...
  const candidates = [
    ...
    { token: process.env.FACEBOOK_PAGE_ACCESS_TOKEN, source: "env" },
  ].filter((candidate) => candidate.token);

  const failedChecks = [];
  for (const candidate of candidates) {
    const check = await validateFacebookPageToken(candidate.token, pageId);
    if (check.ok) return { pageId, pageAccessToken: candidate.token, source: candidate.source };
    failedChecks.push(`${candidate.source}: ${check.error}`);
  }

  const metaConnection = await getProviderConnectionAsync("meta", { orgId });
  if (!metaConnection?.accessToken) {
    const detail = failedChecks.length ? ` Last check: ${failedChecks[0]}` : "";
    return {
      pageId,
      pageAccessToken: "",
      source: "missing",
      error: `No valid Facebook Page access token is available.${detail} Reconnect Meta Business Login to store a durable page token.`,
    };
  }
  ...
  if (page?.pageAccessToken && page?.id) {
    await saveTokensAsync(
      "facebook",
      {
        ...
        assets,
        connectedVia: "meta-oauth-auto-refresh",
      },
      { orgId }
    );
    return { pageId: page.id, pageAccessToken: page.pageAccessToken, source: "meta-refresh" };
  }
  ...
  return {
    ...
    pageAccessToken: "",
    source: "missing",
    error: "Meta Business Login did not return a Facebook Page access token for the configured page.",
  };
}
...
export async function publishVariants({ platform, content, linkedinContent, xContent, facebookContent, blueskyContent, redditContent, redditTitle }) {
  ...
  if (platform === "facebook" || platform === "all") {
    const facebook = await getFacebookPublishCredentials();
