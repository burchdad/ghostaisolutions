...
export async function listProviderConnectionsAsync(platform) {
  if (hasDatabaseConfig()) {
    await ensureDb();
    const result = await query(
      "SELECT org_id, record FROM provider_tokens WHERE platform = $1 ORDER BY updated_at DESC",
      [platform]
    );
    return result.rows
      .map((row) => decodeLegacyOrScopedRecord(row.record))
      .filter(Boolean);
  }

  return listProviderConnections(platform);
}
