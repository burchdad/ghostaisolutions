...
import { SOCIAL_PLATFORMS, normalizeVariant } from "@/lib/socialRepurpose";

const GITHUB_API_BASE = "https://api.github.com";
const LOCAL_DRAFT_DIR = process.env.VERCEL
  ? path.join("/tmp", ".internal", "social-drafts")
...
async function ensureDb() {
  ...
  `);
  await query("CREATE INDEX IF NOT EXISTS social_drafts_status_idx ON social_drafts (status)");
  await query("CREATE INDEX IF NOT EXISTS social_drafts_updated_at_idx ON social_drafts (updated_at DESC)");
  dbReady = true;
}
...
async function listDraftsDb() {
  await ensureDb();
  const result = await query(
    "SELECT data FROM social_drafts ORDER BY updated_at DESC, created_at DESC"
  );
  return result.rows.map(rowToDraft).filter(Boolean);
}

async function getDraftDb(id) {
  await ensureDb();
  const result = await query("SELECT data FROM social_drafts WHERE id = $1", [id]);
  return rowToDraft(result.rows[0]);
}

async function writeDraftDb(draft) {
  await ensureDb();
  await query(
    `INSERT INTO social_drafts (id, slug, status, source_type, data, created_at, updated_at)
...
function normalizePlatformVariants(platformVariants = {}) {
  return Object.fromEntries(
    SOCIAL_PLATFORMS.map((platform) => [platform, normalizeVariant(platform, platformVariants[platform])])
  );
}
