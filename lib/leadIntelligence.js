...
function normalizeOutreachBody(text = "") {
  const stripped = String(text)
    .replace(/\[your\s+name\]/gi, "")
    .replace(/\[your\s+position\]/gi, "")
    .replace(/\[your\s+studio\]/gi, "")
    .replace(/\[your\s+contact\s+information\]/gi, "")
    .trim();

  const optOutLine = "If this is not relevant, reply \"stop\" and I will not follow up.";
  if (stripped.toLowerCase().includes("reply \"stop\"") || stripped.toLowerCase().includes("reply 'stop'")) {
    return stripped;
  }
...
export async function generateOutreachDraft(lead) {
  ...
  const senderName = process.env.OUTREACH_SENDER_NAME || process.env.OUTREACH_SEND_NAME || "Alex Johnson";
  const senderRole = process.env.OUTREACH_SENDER_ROLE || "Ghost AI Solutions";
  ...
  const fallbackBody = [
    ...
    "",
    `I took a quick look at ${lead.companyName}${lead.website ? ` (${lead.website})` : ""} and saw a few opportunities to improve lead qualification and follow-up speed.`,
    "",
    "For teams with similar setup, we usually help with:",
    ...
    "",
    `If useful, start here and I will send back a short website + lead path audit: ${intakeUrl}`,
    "",
    "Best,",
    senderName,
    senderRole,
  ...
  const prompt = `You are writing a concise, high-quality first-touch cold outreach email for a boutique AI systems studio.
...
Rules:
- This is FIRST outreach. Do not imply they already contacted us, are a customer, or are in a current project.
- Keep it 90-150 words.
- Tone: direct, practical, not hypey, no exaggerated claims.
- Mention exactly 1-2 concrete observations from the signals.
- Avoid attachments, emojis, and marketing buzzwords.
- Use this exact CTA link once: ${intakeUrl}
- The CTA should invite them to start a free website/lead path audit, not book a call immediately.
- Signature should be exactly:
  Best,
  ${senderName}
  ${senderRole}
- Include this exact line at the end: If this is not relevant, reply "stop" and I will not follow up.
- Return JSON only with keys: subject, body.`;
  ...
  try {
    ...
    console.warn("OpenAI outreach draft fallback used", {
      companyName: lead.companyName,
      reason: error?.message || String(error),
    });
    return {
      subject: fallbackSubject,
      body: normalizeOutreachBody(fallbackBody),
      generatedAt: new Date().toISOString(),
      model: "template-fallback-openai-network",
  ...
  if (!response.ok) {
    ...
    return {
      ...
      body: normalizeOutreachBody(fallbackBody),
      generatedAt: new Date().toISOString(),
      model: "template-fallback-openai-error",
      providerError: `OpenAI draft generation failed (${response.status}): ${text.slice(0, 500)}`,
    };
  }
  ...
  return {
    ...
    body: normalizeOutreachBody(String(parsed.body).slice(0, 5000)),
    generatedAt: new Date().toISOString(),
    model,
  };
}
...
export async function sendLeadEmail({ to, subject, html, text }) {
  ...
  const replyTo = process.env.OUTREACH_REPLY_TO || process.env.RESEND_REPLY_TO || undefined;
  ...
  if (!response.ok) {
    const providerCode = payload?.name || payload?.error || payload?.code || "resend_error";
