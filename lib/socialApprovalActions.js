...
function summarizePublishResults(results = {}) {
  ...
  if (!entries.length) return "No platform publishing was attempted.";

  return entries
    .map(([platform, result]) => {
      const label = platform.charAt(0).toUpperCase() + platform.slice(1);
      if (result?.success) return `${label}: published`;
...
export async function runSocialApprovalAction({ draftId, action }) {
  ...
  const summary = summarizePublishResults(results.results);

  return {
    ok: results.success,
    partial: !results.success,
    message: `Social draft ${draftId} ${ACTION_LABELS[action]}. ${summary}`,
    draft: saved,
