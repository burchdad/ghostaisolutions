...
function requireMetaAppSecret() {
  const value = getMetaAppSecret();
  if (!value) {
    throw new Error("Missing required environment variable: META_APP_SECRET or FACEBOOK_APP_SECRET");
  }
  return value;
}

function getMetaRedirectUri() {
  const configured = (process.env.META_REDIRECT_URI || process.env.FACEBOOK_REDIRECT_URI || "").trim();
  ...
  return `${siteUrl}/api/meta/oauth/callback`;
}
...
export function selectMetaPage(assets = {}, configuredPageId = process.env.FACEBOOK_PAGE_ID || "") {
  const pages = Array.isArray(assets.pages) ? assets.pages : [];
  const requestedPageId = String(configuredPageId || "").trim();
  if (requestedPageId) {
    return pages.find((page) => String(page.id) === requestedPageId) || null;
  }
  return pages[0] || null;
}
