...
async function createInstallationAccessToken({ owner, repo, appId, privateKey, installationId }) {
  ...
  let resolvedInstallationId = await resolveInstallationId({ owner, repo, jwt, installationId });

  if (!resolvedInstallationId) {
    throw new Error("Unable to resolve GitHub App installation ID");
  }

  async function createTokenForInstallation(id) {
  ...
  let data;
  try {
    data = await createTokenForInstallation(resolvedInstallationId);
  } catch (error) {
    if (!installationId || !/not found/i.test(error?.message || "")) {
      throw error;
    }

    resolvedInstallationId = await resolveInstallationId({ owner, repo, jwt, installationId: "" });
    data = await createTokenForInstallation(resolvedInstallationId);
  }
